...
describe('User controller', () => {
  ...
  describe('Get User by id', () => {
    ...
    it('Get User Error: should return NOT_FOUND error', async () => {
      const error = new HttpExceptionError(
        STATUS_CODE.NOT_FOUND,
        MESSAGES_AUTHENTICATION.USER_NOT_FOUND
      );

      userServices.getUserById = jest.fn().mockResolvedValue(null);
      await userController.getUserById(req as Request, res as Response, next as NextFunction);

      expect(next).toHaveBeenCalledWith(error);
    });

    it.skip('Get User Error: should return INTERNAL_SERVER_ERROR error', async () => {
      const error = new HttpExceptionError(
